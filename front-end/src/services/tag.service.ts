import { SERVICE_CONFIG_ACCESS_KEY, VITE_API_BASE_URL } from "@/constants";
import { setTags } from "@/store/features/tag.features";
import { useAppDispatch } from "@/store/hook";
import { TResponse, TTag } from "@/types";
import { AxiosError } from "axios";
import { APIService } from "./api.service";

export class TagService extends APIService {
  dispatch = useAppDispatch();

  constructor() {
    super(VITE_API_BASE_URL, {
      accessTokenKey: SERVICE_CONFIG_ACCESS_KEY,
    });
  }

  getTags = async (): Promise<TTag[]> => {
    try {
      const { data } = await this.get<TResponse<TTag[]>>(`/tags`);
      this?.dispatch(setTags(data?.data));

      return data?.data;
    } catch (error) {
      if (error instanceof AxiosError) {
        console.error("GET_TAGS_ERROR", error.response);
        return [];
      } else {
        console.error("UNKNOWN_ERROR", error);
        return [];
      }
    }
  };

  getTag = async (tag_id: string): Promise<TTag> => {
    try {
      const { data } = await this.get<TResponse<TTag[]>>(`/tag/${tag_id}`);
      return (data?.data[0] ?? {}) as TTag;
    } catch (error) {
      if (error instanceof AxiosError) {
        console.error("GET_TAG_ERROR", error.response);
        return {} as TTag;
      } else {
        console.error("UNKNOWN_ERROR", error);
        return {} as TTag;
      }
    }
  };

  checkedTag = async (tag_id: string): Promise<TResponse<[]>> => {
    try {
      const { data } = await this.patch<TResponse<[]>>(
        `/tag/${tag_id}/checked`
      );
      return data;
    } catch (error) {
      if (error instanceof AxiosError) {
        console.error("CHECKED_TAG_ERROR", error.response);
        return {
          message:
            error.response?.data?.message ||
            "Failed to checked tag due to an unknown error",
          status: "error",
          data: [],
        };
      } else {
        console.error("UNKNOWN_ERROR", error);
        return {
          message: "Failed to checked tag due to an unknown error",
          status: "error",
          data: [],
        };
      }
    }
  };
}
