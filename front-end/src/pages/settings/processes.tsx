import { SettingsProcessesTable } from "@/components/settings/processes";
...
export const SettingProcessesPage = () => {
    ...
    <div className='p-2 w-full h-full flex flex-col gap-2 overflow-hidden'>
      <div className='w-full rounded-md h-full max-h-full flex flex-col overflow-hidden '>
        <div className=' w-full flex justify-between items-center h-min p-2 bg-white'>
          <p className=' text-lg font-bold'>Processes Settings</p>
        ...
        {accounts?.length > 0 ? (
          <SettingsProcessesTable {...{ search }} />
        ) : (
          <SettingsProcessesTable />
        )}
      </div>
    </div>
  );
